...
class Cova_Integration {
    ...
    public function activate() {
        $this->create_tables(); // This creates or updates all plugin tables including cova_error_logs

        // Explicitly ensure 'context' column in cova_error_logs is LONGTEXT
        global $wpdb;
        $table_name = $wpdb->prefix . 'cova_error_logs';
        $column_exists = $wpdb->get_var("SHOW COLUMNS FROM `$table_name` LIKE 'context'");

        if ($column_exists) {
            // If column exists, ensure it's LONGTEXT
            $wpdb->query("ALTER TABLE `$table_name` MODIFY COLUMN `context` LONGTEXT");
        } else {
            // If column doesn't exist, add it as LONGTEXT
            $wpdb->query("ALTER TABLE `$table_name` ADD COLUMN `context` LONGTEXT AFTER `message`");
        }
